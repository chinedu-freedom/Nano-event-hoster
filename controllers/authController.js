...
const signup = async (req, res) => {
  ...
  try {
    ...
    const verificationLinkHtml = `<p>Please click the link to verify your email address:</p><a href="${process.env.APP_URL}/verify-email/${verificationToken}">Verify Email</a><p>This token expires in 1 hr</p>`;
    try {
      await sendEmail({
        to: newUser.email,
